...
export async function planRoute({ source, destination, disability_type, date, time, fast_mode }, token = null) {
  const headers = { 'Content-Type': 'application/json' }
  if (token) headers['Authorization'] = `Bearer ${token}`
  const res = await fetch(`${BASE}/process`, {
    method: 'POST',
    headers,
    body: JSON.stringify({ source, destination, disability_type, date, time, fast_mode: !!fast_mode }),
  })
